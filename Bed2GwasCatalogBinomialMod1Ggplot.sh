...
pdf(\"output.pdf\",width=14, height=12)

p<- ggplot(data, aes(x=data\$FC, y=data\$LogP,label=row.names(data))) + geom_point(shape=19, alpha=1/8, color=\"red\", aes(size=data\$\"Phenotype SNPs\")) + xlab(\"Fold change\") + ylab(\"-log P-value\") + ggtitle (\"GWAS SNPs enrichment - binomial test\") + xlim(xmin-1, xmax)

print(p+ geom_text_repel(aes(label=row.names(data), colour = data\$\"Category\", size=1), show.legend=F) + scale_colour_hue(name=\"Category\") + labs(size=\"Phenotype SNPs\", color=\"Category\") + scale_size(range = c(10,40)) + theme(axis.text=element_text(size=26), axis.title=element_text(size=28,face=\"bold\")) + scale_color_manual(values = c(wes_palette(\"Cavalcanti1\"), wes_palette(\"Royal1\"), wes_palette(\"GrandBudapest2\"), wes_palette(\"Royal2\"), wes_palette(\"Darjeeling2\"), wes_palette(\"Zissou1\")))) 
